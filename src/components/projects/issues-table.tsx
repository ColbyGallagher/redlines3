...
import { ChevronDown, Filter, Search, ArrowUpDown, Check, Loader2 } from "lucide-react"
...
export function IssuesTable({ issues, summary }: IssuesTableProps) {
  ...
  const handleBulkUpdate = async (field: string, value: string) => {
    ...
    try {
      ...
      const res = await bulkUpdateIssues(Array.from(selectedIds), summary.project.id, { [dbField]: dbValue })
          ...
          <Table>
            ...
            <TableBody>
              {filteredIssues.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={10} className="text-muted-foreground text-center py-10">
